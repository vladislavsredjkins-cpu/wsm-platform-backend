...
export default function RefereeCompetitions() {
  ...
  const loadDivisions = async (compId) => {
    ...
    const res = await api.get(`/divisions/competition/${compId}`);
  ...
  return (
    <Layout>
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{ color: '#111', fontSize: '22px', fontWeight: '700', margin: '0 0 4px' }}>Referee Portal</h1>
        <p style={{ color: '#666', fontSize: '13px', margin: 0 }}>Select competition and division to enter results</p>
      </div>
        ...
        {competitions.map(c => (
          <div key={c.id}>
            <div
              ...
              style={{ background: '#fff', border: `1px solid ${expanded === c.id ? gold : '#e0e0e0'}`, borderRadius: '4px', padding: '16px 24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
              onMouseEnter={e => e.currentTarget.style.borderColor = gold}
              onMouseLeave={e => e.currentTarget.style.borderColor = expanded === c.id ? gold : '#e0e0e0'}
            >
              <div>
                <div style={{ color: '#111', fontWeight: '600', fontSize: '15px' }}>{c.name}</div>
            ...
            {expanded === c.id && (
              <div style={{ borderLeft: `2px solid ${gold}`, marginLeft: '24px', paddingLeft: '16px', marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {!divisions[c.id] && <p style={{ color: '#666', padding: '12px 0', fontSize: '13px' }}>Loading divisions...</p>}
                {divisions[c.id]?.map(div => (
                  <div
                    key={div.id}
                    onClick={() => navigate(`/referee/${div.id}/disciplines`)}
                    style={{ background: '#f7f4ef', border: '1px solid #e8e8e8', borderRadius: '3px', padding: '12px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = gold}
                    onMouseLeave={e => e.currentTarget.style.borderColor = '#e8e8e8'}
                  >
                    <div>
                      <span style={{ color: '#111', fontWeight: '600' }}>{div.name}</span>
                      {div.gender && <span style={{ color: '#666', fontSize: '12px', marginLeft: '8px' }}>{div.gender}</span>}
                    </div>
