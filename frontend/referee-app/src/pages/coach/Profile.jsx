...
export default function CoachProfile() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/coaches/${user.coach_id}/profile`)
  ...
  const save = async () => {
    ...
    try {
      ...
      await axios.patch(`${API}/coaches/${user.coach_id}`, form, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const uploadPhoto = async (file) => {
    ...
    await axios.post(`${API}/coaches/${user.coach_id}/photo`, fd, { headers: { Authorization: `Bearer ${token}` } });
    window.location.reload();
  };

  if (loading) return <Layout><p style={{color:'#555'}}>Loading...</p></Layout>;
  if (!user?.coach_id) return <Layout><p style={{color:'#ff5252'}}>No judge profile linked.</p></Layout>;

  return (
    <Layout>
      <div style={{marginBottom:'32px'}}>
        <div style={{color:gold,fontSize:'11px',letterSpacing:'3px',marginBottom:'8px'}}>COACH CABINET</div>
